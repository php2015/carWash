    ...
    */
    ...
    public function radioChange(){
        ...
        if(!file_exists($path)){
            ...
            $this->SplFileInfo = new \SplFileInfo($result['path']);
            $this->img = new Image($this->SplFileInfo);
            $this->property['width'] = $this->img->width();
            $this->property['height'] = $this->img->height();
    ...
    public function save($data){
        ...
        foreach($data as $key=>$value){
            ...
            if($value['type'] != 0){
                ...
                $find['handle_type'] = $value['type'];
                $find['cut_size'] = array('IN',$this->config['cut_size']);
                $id_array = Db::name('admin_attachment')->where($find)->field('id')->select();
    ...
    public function cut($path,$name,$type){
        ...
        foreach($cut_size as $key=>$value){
            ...
            /*检查是否有记录*/
            if($result){
                $check = Db::name('admin_attachment')
                    ->where('pid','eq',$result['id']) //原图id
                    ->where('cut_size','eq',$value)     //尺寸
                    ->where('handle_type','eq',$type)      //切图类型
                    ->where('width='.$this->config['width'])    //新图宽
                    ->where('height='.$this->config['height'])  //新图高
                    ->find();
                /*已有该切图的记录,则什么都不做*/
                if($check){

                }else{
                    $info['name'] = $value.'_'.$assembly.'_'.$this->config['width'].'_'.$this->config['height'].'_'.$name;
                    $info['pid'] = $result['id'];
                    $info['path'] = $new_path;
                    $info['handle_type'] = $type;
                    $info['cut_size'] = $value;
                    $info['width'] = $this->config['width'];
                    $info['height'] = $this->config['height'];
                    $info['create_time'] = time();
                    $info['mime'] = $result['mime'];
                    $info['ext'] = $result['ext'];
                    Db::name('admin_attachment')->insert($info);
                }
    ...
    public function getPreview($pic_array){
        if(is_array(json_decode($pic_array,true))){
            echo json_encode(array());
            exit;
        }
        $array = explode(',',$pic_array);

        $new_array = array();

        /*定位原图路径*/
        foreach($array as $key => &$value){
            $result = Db::name('admin_attachment')->where('id','eq',$value)->find();
            $result_temp = Db::name('admin_attachment')->where('id','eq',$result['pid'])->find();


            if(!$result_temp['path'] && $result['path'] && empty($result['pid'])){
                $new_array[$key]['path'] = $result['path'];
            }else{
            ...
            $new_array[$key]['type'] = $result['handle_type']?$result['handle_type']:'6';

            $new_array[$key]['id'] = $value;
        ...
        foreach($new_array as $E => $F){
            /*10.26*/
            $name_array = explode('/',$F['path']);
            $this->name = end($name_array);
            $update['name'] = $this->name;
            Db::name('admin_attachment')->where('id','eq',$F['id'])->update($update);
            /*10.26*/
    ...
    public function editImgProcess($array){
        ...
        }else{ //如果没有图片,则显示系统默认图
            $result['type'] = '6';
            $result['temp_path'] = '';
            $result['path'] = config('public_static_path').'admin/img/none.png';
            $result['id'] = '';
            return $result;
        }
