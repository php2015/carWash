...
class Admin extends Common
{
    ...
    protected function _initialize()
    {
        ...
        // 写入提现手续费
        $isset = Db::name('cash_scale')->find(); 
        if(empty($isset)){
            $result["create_time"] = time();
            $result['fee'] = config('app.commission_rate');
            Db::name('cash_scale')->insert($result); 
        }
    }
...
//        _system_check();
     ...
     * 动态修改图片上传相关参数
     * @param $title  [标题]
     * @param $number [允许的上传数量]
     * @param $width  [图片宽度]
     * @param $height [图片高度]
     * @return mixed
     */
    public function setUploadImgParams($title , $number, $width, $height)
    {
        $params = Config::get('app.uploads');
        if(empty($title) || empty($number) || empty($width) || empty($height)){
            return $params;
        }
